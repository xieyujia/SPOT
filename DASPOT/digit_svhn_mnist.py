...
parser.add_argument('--source', default='svhn', help='mnist | mnistm | usps | svhn')
parser.add_argument('--target', default='mnist', help='mnist | mnistm | usps | svhn')
...
parser.add_argument('--d_iter', type=int, default=5, help='train Discriminater for d_iter times per iteration')
...
parser.add_argument('--netPlossscale2', type=float, default=0.1, help='netPlossscale2')
...
parser.add_argument('--softmaxaligntime', type=int, default=5000, help='when to begin align softmax')
...
def main():
    ...
    def createDataset(dataname, train):
        ...
        if dataname == "svhn":
            return dset.SVHN(root=opt.dataroot+'/svhn', split=("extra" if train else "test"), download=True,
                                transform=transforms.Compose([
                                    transforms.ToTensor()
                                ]))
    ...
    import models.model_svhn_mnist as models
    from utils import DataAttDict

    netP = models.CoDis32x32(*DataAttDict[opt.source],*DataAttDict[opt.target]).to(device)
    netD_aux = nn.Sequential(
                    nn.Conv2d(128, 20, kernel_size=3, stride=2, padding=1),
                    nn.PReLU(),
                    nn.Conv2d(20, 1, kernel_size=8, stride=1, padding=0)
                ).to(device)
    ...
    netG = models.CoGen32x32(*DataAttDict[opt.source],*DataAttDict[opt.target], zsize=opt.zsize).to(device)
    ...
    for i in range(opt.niter):
        ...
        for d_iter in range(opt.d_iter):
            ...
            errD_xy += ganloss(-netD_aux2(x_rep)) * opt.netDlossscale
            errD_xy += ganloss(netD_aux2(y_rep)) * opt.netDlossscale
            ...
            #         errP_x = criterion(ys_out_fake,ys_label) * opt.netPlossscale2


            optloss = ((x_rep_fake-y_rep_fake)**2).sum()/opt.batchSize * opt.OPTlossscale
            errP_x += criterion(x_out,x_label) * opt.netPlossscale
            # GAN training for y
            lossP = errD_x_real+errD_x_fake+errP_x+errD_y_real+errD_y_fake+optloss+errD_xy
            lossP.backward()
            optimizerP.step()
