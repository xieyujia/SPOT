...
parser.add_argument('--softmaxaligntime', type=int, default=10000, help='when to begin align softmax')
...
def main():
    ...
    import models.model_mnist_usps_withmapping as models
    ...
    for i in range(opt.niter):
        ...
        for d_iter in range(opt.d_iter):
            ...
            optloss = ((x_rep_fake-y_rep_fake)**2).sum()/opt.batchSize * opt.OPTlossscale + ((x_rep-y_rep_fake2)**2).sum()/opt.batchSize * opt.OPTlossscale
        ...
        y_out_d_fake2,y_rep_fake2 = netP.ganforward_t2(y_fake2)
        _,x_rep = netP.ganforward_s(x)
        x_rep = x_rep.detach()

        errD_x_fake = ganloss(x_out_d_fake) * opt.netDlossscale
        errD_y_fake = ganloss(y_out_d_fake) * opt.netDlossscale + ganloss(y_out_d_fake2) * opt.netDlossscale
        D_x_fake = x_out_d_fake.mean().item()
        D_y_fake = y_out_d_fake.mean().item()
        ...
        if i%opt.testiter == 0:
            ...
            fake_images2 = torch.cat((x, y_fake2), 3)
            torchvision.utils.save_image(fake_images2.data, opt.experiment +'/fakeimage2_%05d.jpg'%(i))
            show_tsne(
